...
const commandsPath = rootPath + "\\src\\commands\\data\\"; //Directory that contains command data files
...
/******************** Get token, client ID, guild ID ********************/

console.log( `Accessing Discord client ID, token, developer guild ID at "${configPath}"` )
...
guildId = require( configPath ).guildId;
if ( guildId ) console.log( "Found developer guild ID" );
else {
    console.log( "ERROR: Unable to find guild ID. Has it been set?" );
    process.exit();
}//end if-else
...
fs.access( commandsPath, fs.constants.R_OK, ( error ) => {
    ...
    console.log( `Started refreshing application slash commands for guild with ID "${guildId}"...` );
    ...
    rest.put(
        ...
        .then( () => {
            console.log( "Successfully refreshed application slash commands for the specified guild!" );
        } )
        .catch( ( error ) => {
